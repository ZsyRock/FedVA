...
def run_model_poison_exp(num_poisoned_workers, kwargs, strategy, experiment_id):
    def config_modifier(args):
        # ===== 训练基本参数 =====
        args.batch_size = 16 # 模型投毒一般不受small batch限制，可比 lf.py 稍大
...
if __name__ == "__main__":
    ...
    # ===== 关键实验开关 =====
    NUM_POISONED_WORKERS = 10   # 恶意客户端个数
    KWARGS = {
        "NUM_WORKERS_PER_ROUND": 20,   # 每轮参与客户端数（随机）
        "TOTAL_NUM_WORKERS": 50,   # 总客户端数（用于上面 args.num_workers）
